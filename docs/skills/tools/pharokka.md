---
title: Pharokka
description: 快速噬菌体基因组注释工具，自动检测 tRNA、tmRNA 和基因功能
---

# Pharokka

> 快速噬菌体基因组注释工具，自动检测 tRNA、tmRNA 和基因功能

## 基本信息

| 属性 | 值 |
|------|-----|
| **类别** | viral-analysis |
| **难度** | ⭐⭐ 中级 |
| **语言** | N/A |
| **GitHub Stars** | 0 |
| **标签** | phage, annotation, fast, automated |

## 链接

- 📦 **代码仓库**: [https://github.com/gbouras13/pharokka](https://github.com/gbouras13/pharokka)
- 📄 **论文**: [https://doi.org/10.1093/nargab/lqad054](https://doi.org/10.1093/nargab/lqad054)

## 安装

```bash
conda install -c bioconda pharokka
```

## 简介

快速噬菌体基因组注释工具，自动检测 tRNA、tmRNA 和基因功能

## 使用示例

> 📝 更多使用示例请参考官方文档。

## 参考资源

- 📖 **官方文档**: [https://github.com/gbouras13/pharokka](https://github.com/gbouras13/pharokka)
- 🎓 **教程**: [https://pharokka.readthedocs.io/en/latest/](https://pharokka.readthedocs.io/en/latest/)
- 📄 **论文**: Bouras et al. (2023) Pharokka: a fast scalable bacteriophage annotation tool. *Bioinformatics*. DOI: [10.1093/nargab/lqad054](https://doi.org/10.1093/nargab/lqad054)

### 相关工具

- [VirSorter 2](./virsorter2)
- PHASTER
- CheckV

### 相关概念

- [Genome Resolved Metagenomics](../concepts/genome-resolved-metagenomics)

---

*最后更新: 2026-03-30*
