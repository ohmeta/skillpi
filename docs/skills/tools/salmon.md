---
title: Salmon
description: 超快速转录本定量工具，支持映射和无比对模式
---

# Salmon

> 超快速转录本定量工具，支持映射和无比对模式

## 基本信息

| 属性 | 值 |
|------|-----|
| **类别** | quantification |
| **难度** | ⭐⭐ 中级 |
| **语言** | N/A |
| **GitHub Stars** | 0 |
| **标签** | quantification, transcriptomics, metatranscriptomics, fast |

## 链接

- 📦 **代码仓库**: [https://github.com/COMBINE-lab/salmon](https://github.com/COMBINE-lab/salmon)
- 📄 **论文**: [https://doi.org/10.1038/nmeth.4197](https://doi.org/10.1038/nmeth.4197)

## 安装

```bash
conda install -c bioconda salmon
```

## 简介

超快速转录本定量工具，支持映射和无比对模式

## 使用示例

> 📝 更多使用示例请参考官方文档。

## 参考资源

- 📖 **官方文档**: [https://salmon.readthedocs.io/en/latest/](https://salmon.readthedocs.io/en/latest/)
- 🎓 **教程**: [https://salmon.readthedocs.io/en/latest/salmon.html](https://salmon.readthedocs.io/en/latest/salmon.html)
- 📄 **论文**: Patro et al. (2017) Salmon provides fast and bias-aware quantification of transcript expression. *Nature Methods*. DOI: [10.1038/nmeth.4197](https://doi.org/10.1038/nmeth.4197)

### 相关工具

- [Kallisto](./kallisto)
- [STAR](./star)
- [featureCounts](./featurecounts)

### 相关概念

- [Metatranscriptomics](../concepts/metatranscriptomics)

---

*最后更新: 2026-03-30*
