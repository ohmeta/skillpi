...
@click.option("--type", "skill_type", type=click.Choice(["tool", "workflow", "concept"]), 
              required=True, help="技能类型")
...
@click.option("--format", "file_format", type=click.Choice(["json", "yaml", "csv"]), default="json", help="输入文件格式")
@click.option("--output", "-o", default="data/skills", help="输出目录")
def import_skills(input_file: str, file_format: str, output: str):
    """导入技能数据"""
    console.print(f"[green]导入技能数据...[/green]")
...
@click.option("--format", "file_format", type=click.Choice(["json", "yaml", "csv", "markdown"]), default="json", help="输出文件格式")
def export_skills(input_file: str, output: str, file_format: str):
    """导出技能数据"""
    console.print(f"[green]导出技能数据...[/green]")
    console.print(f"输入文件：{input_file}")
...
def assess(input_file: str, auto_apply: bool, output: str):
    """评估技能难度"""
    console.print(f"[green]评估技能难度...[/green]")
    ...
    try:
        ...
        for skill in skills:
            ...
            console.print(f"  [{color}]{status}[/] {skill.data.name}: {old_level.value} → {new_level.value}")
